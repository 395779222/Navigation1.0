...
<%@ taglib prefix="s" uri="/struts-tags"%><html>	
  ...
  <head>
    <title>IPTV</title>
    <meta http-equiv="Content-Type" content="720*576;text/html;charset=UTF-8"/>
...
　　    　　   alert("按键码: " + keycode + " 字符: " + realkey);*/
		...
		switch(key_code) {
			...
			//up
			case 1:case 33:case 38:doUp();return 0;break;
			//down
			case 2:case 34:case 40:doDown();return 0;break;
			//left		
...
<body  bgColor="transparent" leftmargin="0" topmargin="0" marginwidth="0" marginheight="0" onLoad="onload();" scroll="no" >
    <div style="position:absolute; left:0px; top:0px; width:720px; height:576px;"><img src="<%=request.getContextPath()%>/images/sdsy/sd.jpg" width="720" height="576"  /></div>
	<!--通知链接-->
...
<!-- 	<div style="position:absolute; top:125px; left:1049px;" ><img src="<%=request.getContextPath()%>/${adIcon}" ></div> -->
	<!--导航-->
	<span id ="btn"> 
	<input id="testkeyevent" name="testkeyevent"   onkeypress="keypress()" display="none"/>
	<s:iterator value="listNavigation" id="bean">
		<s:if test="%{#bean.location%15==0}">
			<div id="${bean.location}" style="position:absolute;display:none;
			 	top:301px; 
			 	left:567px;">
			 	<a href="${bean.url }"><img id="<%=request.getContextPath()%>/${bean.icon2 }" src="<%=request.getContextPath()%>/${bean.icon }" name="${bean.namena }" width="105" height="75" border="0">
			 	</a>
			</div>
		</s:if>
		...
		<s:elseif test="%{#bean.location==1}">
			<div id="${bean.location}" style="position:absolute;
				 top:92px; 
				 left:49px;display:none" >
				 <a href="${bean.url }"><img id="<%=request.getContextPath()%>/${bean.icon}" src="<%=request.getContextPath()%>/${bean.icon2}" name="${bean.namena }" width="105" height="75" border="0">
				 </a>
			</div>
		</s:elseif>
		<s:elseif test="%{#bean.location%15/5<1}">
			<div id="${bean.location}" style="position:absolute;
				 top:92px; 
				 left:${bean.location%15%5*130-81}px;display:none" >
				 <a href="${bean.url }"><img id="<%=request.getContextPath()%>/${bean.icon2}" src="<%=request.getContextPath()%>/${bean.icon }" name="${bean.namena }"width="105" height="75" border="0">
				 </a>
			</div>
		</s:elseif>
		...
		<s:elseif test="%{#bean.location%15/5<81}">
			<div id="${bean.location}" style="position:absolute;
				 top:301px; 
				 left:${bean.location%15%5*130-81}px;display:none" >
				 <a href="${bean.url }"><img id="<%=request.getContextPath()%>/${bean.icon2}" src="<%=request.getContextPath()%>/${bean.icon }" name="${bean.namena }" width="105" height="75" border="0">
   	...
   	<s:iterator value="shortList" id="bean">
   		<s:if test="%{#bean.key==1}">
   			<div style="position:absolute; top:420px; left:50px;dispaly:none" ><img src="<%=request.getContextPath()%>/${bean.icon }"  width="120" height="68" border="0"></div>
   		</s:if>
   		<s:elseif test="%{#bean.key==2}">
   			<div style="position:absolute; top:420px; left:180px;dispaly:none" ><img src="<%=request.getContextPath()%>/${bean.icon }"  width="120" height="68" border="0"></div>
   		</s:elseif>
   		<s:elseif test="%{#bean.key==3}">
   			<div style="position:absolute; top:420px; left:309px; width: 169px;dispaly:none" ><img src="<%=request.getContextPath()%>/${bean.icon }"  width="120" height="68" border="0"></div>	
   		</s:elseif>
   	</s:iterator>
	<div style="position:absolute; top:436px; left:480px; width: 118px; height: 25px; text-align:center;" class="font_18" >第<span id="pageNum"></span>/${maxNum}页<br></div>
   	...
   	<div style="position:absolute; left:300px; top:50px;">
		  预览页面按此处翻页：<input type="button" value="上一页" onclick="doPageUp()" >
		 <input type="button" value="下一页" onclick="doPageDown()">
	</div>
  </body>
</html>
